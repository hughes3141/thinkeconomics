...
function getMCQquestionDetails($id = null, $questionNo = null, $topic = null) {
  ...
  */
  ...
  $sql ="SELECT q.id, q.No, q.Answer, q.Topic, q.topics, q.keywords, q.question, q.options, q.explanation, q.examBoard, q.component, q.assetId, q.unitName, q.qualLevel, q.textOnly, a.path
...
function updateMCQquestion($id, $userId, $explanation, $question, $optionsJSON, $topic, $topics, $answer, $keywords, $textOnly) {
  /*
  ...
  */
  ...
  $sql = "UPDATE question_bank_3
          SET question = ?, options = ?, Topic = ?, topics = ?, Answer = ?, keywords = ?, textOnly = ?
          WHERE id = ?";
  $stmt=$conn->prepare($sql);
  $stmt->bind_param("ssssssii", $question, $optionsJSON, $topic, $topics, $answer, $keywords, $textOnly, $id);
  $stmt->execute();
