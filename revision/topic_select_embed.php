        ...
        <div id="accordion-collapse" data-accordion="collapse" class="my-2 border rounded-xl border-gray-200 mt-2">
          ...
          <div id="accordion-collapse-body-1" class="hidden" aria-labelledby="accordion-collapse-heading-1">
            <div class="p-5  border-b-0 border-gray-200 dark:border-gray-700 dark:bg-gray-900">
              <div>
                ...
                <?php
                  $examBoardSelector = empty($subjectIdSet);
                ?>
                <div class= "<?=$examBoardSelector ? " hidden " : ""?>">
                  <label for="examBoardId">Exam Board:</label>
                  <select class="mb-3 " id="examBoardId" name="examBoardId" onchange="this.form.submit()" >
                    <option value=""></option>
                    <?php
          ...
          <div id="accordion-collapse-body-2" class="hidden rounded-b-xl" aria-labelledby="accordion-collapse-heading-2">
            <div class="p-5  border-gray-200 dark:border-gray-700">
              <div class="grid gap-2 grid-cols-1 md:grid-cols-4">
                <?php
                  ...
                  foreach($topicsArray as $topic) {
                    ?>
                    <div>
                      <input type="checkbox" id="topic_<?=htmlspecialchars($topic['id'])?>" class= "topicSelector" value="<?=htmlspecialchars($topic['id'])?>" onchange="topicAggregate();" <?php
                        ...
                        if(!is_null($topicIdsArray) && in_array($topic['id'], $topicIdsArray)) {
                          echo " checked ";
                        } 
                      ?>>
                      <label for = "topic_<?=htmlspecialchars($topic['id'])?>" ><?=htmlspecialchars($topic['code'])?> <?=htmlspecialchars($topic['name'])?></label>
                    </div>
                    <?php
                  }
              ...
              ?>
                <input type="hidden" name="topicIds" id="topicSelect">
